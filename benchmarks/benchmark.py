    ...
    for _ in range(size - unique_count):
        ...
        for _ in range(change_count):
            ...
            if random.random() < 0.33:  # Delete
                ...
                duplicate[pos] = random.choice(string.ascii_letters + string.digits + ' ')
            else:  # Insert
                duplicate.insert(pos, random.choice(string.ascii_letters + string.digits + ' '))
...
def benchmark_pandas(data: List[str], threshold: int) -> Tuple[float, int]:
    ...
    for i in range(len(df)):
        ...
        # Find similar items
        similar_indices = [j for j in range(len(df)) 
                ...
                for lib_name, benchmark_func in benchmarks.items():
                    for _ in range(runs):
                        try:
                            ...
                            print(f"Error in {lib_name} with size={size}, threshold={threshold}: {e}")
...
def plot_results(results: Dict[str, Dict[str, List[float]]], output_dir: str):
    ...
    for threshold in thresholds:
        ...
        for lib_name in results.keys():
            ...
            indices = [i for i, t in enumerate(results[lib_name]['threshold']) if t == threshold]
            ...
            for size in unique_sizes:
                ...
                avg_time = sum(filtered_times[i] for i in size_indices) / len(size_indices)
    ...
    for size in sizes:
        ...
        for lib_name in results.keys():
            ...
            for threshold in unique_thresholds:
                threshold_indices = [i for i, t in enumerate(filtered_thresholds) if t == threshold]
                avg_time = sum(filtered_times[i] for i in threshold_indices) / len(threshold_indices)
                avg_times.append(avg_time)
    ...
    for lib_name in results.keys():
        ...
        for size in unique_sizes:
            size_indices = [i for i, s in enumerate(results[lib_name]['size']) if s == size]
            avg_time = sum(results[lib_name]['time'][i] for i in size_indices) / len(size_indices)
            avg_times.append(avg_time)
...
def generate_markdown_report(results: Dict[str, Dict[str, List[float]]], output_dir: str):
    """
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("## Summary\n\n")
        f.write("This report compares the performance of fast-dedupe with other deduplication libraries:\n\n")
        ...
        for threshold in thresholds:
            ...
            # Create table header
            f.write("| Dataset Size | fast-dedupe (s) | pandas (s) | fuzzywuzzy (s) | Speedup vs pandas | Speedup vs fuzzywuzzy |\n")
            f.write("|--------------|-----------------|------------|----------------|-------------------|----------------------|\n")
            ...
            for size in sizes:
                ...
                for lib_name in results.keys():
                    indices = [i for i, (s, t) in enumerate(zip(results[lib_name]['size'], results[lib_name]['threshold'])) 
                              if s == size and t == threshold]
                    
                    if indices:
                        avg_times[lib_name] = sum(results[lib_name]['time'][i] for i in indices) / len(indices)
                    else:
                        avg_times[lib_name] = float('inf')
                
                # Calculate speedups
                speedup_pandas = avg_times['pandas'] / avg_times['fastdedupe'] if avg_times['fastdedupe'] > 0 else float('inf')
                speedup_fuzzywuzzy = avg_times['fuzzywuzzy'] / avg_times['fastdedupe'] if avg_times['fastdedupe'] > 0 else float('inf')
                
                # Format times and speedups
                times = {lib: f"{t:.4f}" if t != float('inf') else "N/A" for lib, t in avg_times.items()}
                speedups = {
                    'pandas': f"{speedup_pandas:.2f}x" if speedup_pandas != float('inf') else "N/A",
                    'fuzzywuzzy': f"{speedup_fuzzywuzzy:.2f}x" if speedup_fuzzywuzzy != float('inf') else "N/A"
                }
                
                # Add row to table
                f.write(f"| {size} | {times['fastdedupe']} | {times['pandas']} | {times['fuzzywuzzy']} | {speedups['pandas']} | {speedups['fuzzywuzzy']} |\n")
            
            f.write("\n")
            f.write(f"![Time vs Size (Threshold: {threshold})](time_vs_size_threshold_{threshold}.png)\n\n")
        
        f.write("## Conclusion\n\n")
        
        # Calculate overall average speedups
        fastdedupe_times = [t for t in results['fastdedupe']['time'] if t != float('inf')]
        pandas_times = [t for t in results['pandas']['time'] if t != float('inf')]
        fuzzywuzzy_times = [t for t in results['fuzzywuzzy']['time'] if t != float('inf')]
        
        if fastdedupe_times and pandas_times:
            avg_speedup_pandas = sum(pandas_times) / sum(fastdedupe_times)
            f.write(f"- On average, fast-dedupe is **{avg_speedup_pandas:.2f}x faster than pandas**\n")
        
        if fastdedupe_times and fuzzywuzzy_times:
            avg_speedup_fuzzywuzzy = sum(fuzzywuzzy_times) / sum(fastdedupe_times)
            f.write(f"- On average, fast-dedupe is **{avg_speedup_fuzzywuzzy:.2f}x faster than fuzzywuzzy**\n")
        
        f.write("\nThese benchmarks demonstrate that fast-dedupe provides significant performance improvements ")
        f.write("over other popular libraries for fuzzy string matching and deduplication tasks.\n")
...
def main():
    ...
    parser.add_argument("--thresholds", type=int, nargs="+", default=[70, 80, 90], 
    ...
    parser.add_argument("--variation-level", type=float, default=0.2, 
