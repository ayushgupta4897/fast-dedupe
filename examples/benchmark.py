...
def generate_dataset(
    size: int, duplicate_ratio: float = 0.3, variation_level: int = 2
) -> List[str]:
    """
    ...
    for _ in range(duplicate_count):
        ...
        if variation_level == 0:
            ...
            # Case variation
            duplicate = "".join(
                c.lower() if random.random() < 0.3 else c for c in original
            )
...
def benchmark(
    dataset_sizes: List[int], thresholds: List[int], variation_levels: List[int]
) -> None:
    """
    ...
    print(
        f"{'Size':<10} {'Threshold':<10} {'Variation':<10} {'Time (s)':<10} "
    ...
    for size in dataset_sizes:
        for variation in variation_levels:
            ...
            dataset = generate_dataset(
                size, duplicate_ratio=0.3, variation_level=variation
            )
            ...
            for threshold in thresholds:
                ...
                print(
                    f"{size:<10} {threshold:<10} {variation:<10} "
