...
def main():
    ...
    for algorithm in SimilarityAlgorithm:
        clean, dupes = dedupe(general_text, threshold=85, similarity_algorithm=algorithm)
    ...
    clean_jw, dupes_jw = dedupe(names, threshold=85, similarity_algorithm=SimilarityAlgorithm.JARO_WINKLER)
    ...
    clean_soundex, dupes_soundex = dedupe(
        phonetic_examples, threshold=85, similarity_algorithm=SimilarityAlgorithm.SOUNDEX
    )
    ...
    fig2 = visualize_algorithm_comparison(
        string1,
        string2,
        "visualization_examples/algorithm_comparison.png"
    )
