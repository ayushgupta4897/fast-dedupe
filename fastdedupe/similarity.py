...
def get_similarity_function(algorithm: Union[str, SimilarityAlgorithm]) -> SimilarityFunc:
    """
...
def levenshtein_similarity(s1: str, s2: str, **kwargs) -> float:
    """
...
def jaro_winkler_similarity(s1: str, s2: str, **kwargs) -> float:
    """
...
def cosine_ngram_similarity(s1: str, s2: str, n: int = 3, **kwargs) -> float:
    """
...
def jaccard_similarity(s1: str, s2: str, tokenize: bool = True, **kwargs) -> float:
    """
...
def soundex_similarity(s1: str, s2: str, **kwargs) -> float:
    """
...
def visualize_similarity_matrix(
    ...
    max_label_length = 20
    labels = [s if len(s) <= max_label_length else s[:max_label_length-3] + '...' 
              for s in strings]
    ...
    for i in range(n):
        for j in range(n):
            text = ax.text(j, i, f"{similarity_matrix[i, j]:.1f}",
                          ha="center", va="center", color="black" if similarity_matrix[i, j] < 70 else "white")
